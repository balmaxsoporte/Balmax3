...
class EditarPatioEstacionamiento : AppCompatActivity() {
    ...
    private fun setupUI() {
        CoroutineScope(Dispatchers.IO).launch {
            val puesto = db.puestoDao().getPuestosPorPatio(numeroPatio).find { it.id == puestoId }

            runOnUiThread {
                if (puesto != null) {
                    binding.tvTituloEditarPuesto.text = getString(com.example.patiobalmax.R.string.titulo_editar_puesto, puesto.numeroPuesto)

                    // Cargar datos actuales del puesto
